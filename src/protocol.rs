...
pub async fn stls_connect() {
    let host = "192.168.31.169:34081";
    let mut stream = tokio::net::TcpStream::connect(host).await.unwrap();
    // Send CNXN first
    {
        let mut cnxn_message = generate_message(
            A_CNXN,
            A_VERSION,
            MAX_PAYLOAD,
    ...
    // Send STLS packet
    {
        let mut stls_message = generate_message(A_STLS, A_STLS_VERSION, 0, Vec::new());
